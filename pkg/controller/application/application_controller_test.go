...
func TestApplicationDiscovery(t *testing.T) {
	...
	for _, dpl := range dplList.Items {
		...
		g.Expect(annotations[appv1alpha1.AnnotationDiscovered]).To(Equal("true"))
	...
	g.Expect(svcuc.GetAnnotations()[appv1alpha1.AnnotationDiscovered]).To(Equal("true"))

	stsuc, _ := mcDynamicClient.Resource(stsGVR).Namespace(userNamespace).Get(sts.Name, metav1.GetOptions{})
	g.Expect(stsuc.GetAnnotations()[appv1alpha1.AnnotationDiscovered]).To(Equal("true"))
